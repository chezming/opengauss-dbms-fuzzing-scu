...
def opt_callgraph(args, binary):                                            # 使用opt工具构建二进制文件的调用图
...
def construct_callgraph(args):


    tmp_dir = args.temporary_directory
    callgraph_generator = str(args.script_directory) + "/generate_cg.py"
    
    cmd = "python3 " + callgraph_generator + " " + str(tmp_dir)
    subprocess.run(cmd, shell=True)   

    print("[+] Complete generate callgraph!\n") 
    

    next_step(args)                                                             # 将步骤标识符加一，以便下一步处理。
...
def calculating_distances(args):
    ...
    if STEP == 1:
        ...
        log_p = args.temporary_directory / f"step{STEP}.log"
        try:
            r = exec_distance_prog(                                           # 生成cg_distance, 参数分别是，dot格式的cg，目标节点，生成的距离文件，节点名字
                    callgraph,
                    ftargets,
                    callgraph_distance,
                    fnames,
    ...
    def calculate_cfg_distance_from_file(cfg: Path):
        ...
        exec_distance_prog(                                        # 生成cfg_distance
                cfg,
                bbtargets,
                outpath,
                bbnames,
...
def main():
    ...
    parser.add_argument("script_directory", metavar="script-directory",
                        type=is_path_to_dir,
                        help="Directory where this script is located")
    ...
    STEP = get_resume(args)                                                            # 脚本因为其他原因断掉后，可以直接接着上次结果运行

    if not STEP:
        construct_callgraph(args),                                           # 将cfg加入到有向图中，
    calculating_distances(args),                                                       # 使用dijkstra算法计算距离
    done(args)
