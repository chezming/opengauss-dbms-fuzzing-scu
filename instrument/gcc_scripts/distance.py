...
def distance (name):                                        # 这里传入的是每一个函数名
  if not is_cg and name in bb_distance.keys():
    ...
    return
  distance = -1
  for n in find_nodes (name):                               # find_node函数返回graph中该名字对应的节点，即传入函数名的函数所有节点
    d = 0.0
    i = 0
    ...
    if is_cg:
      ...
      for t_name, bb_d in bb_distance.items():
        ...
        for t in find_nodes(t_name):
          try:                                              # 依然是取最短路径的距离
            shortest = nx.dijkstra_path_length(G, n, t)     # 这里的计算实际为(10*func-distance + bb-distance)，然后取调和平均数。
    ...
    if d != 0 and (distance == -1 or distance > i / d) :    # 如果在迭代后，发现有更短的路径，则更新为最短的距离
      distance = i / d

  if distance != -1:                                        # 将距离写入到文件中
    out.write (name)  
    out.write (",")
...
if __name__ == '__main__':
  ...
  print ("\nParsing %s .." % args.dot)
  G = nx.DiGraph(nx.drawing.nx_pydot.read_dot(args.dot))              # 读取图
  print (nx.info(G))

  is_cg = "Call graph" in nx.info(G)                                  # 判断读进来的图是cg还是cfg
  ...
  if not is_cg :                                                     
    ...
    if args.cg_distance is None:                                      # 首先需要函数维度的距离
      print ("Specify file containing CG-level distance (-c).")
      exit(1)

    elif args.cg_callsites is None:                                   # 以及基本块的callsite，即哪些基本块调用了哪些函数
      ...
      with open(args.cg_distance, 'r') as f:                          # 加载函数维度距离
      ...
      with open(args.cg_callsites, 'r') as f:                         # 初始化基本块距离为函数维度距离，取最小的值
        for l in f.readlines():
          s = l.strip().split(",")
          if find_nodes(s[0]):
  ...
  # Process as CallGraph
    ...
    with open(args.targets, "r") as f:
      targets = []                                                  # 这里的target是Ftargets文件，即目标点所在的函数
