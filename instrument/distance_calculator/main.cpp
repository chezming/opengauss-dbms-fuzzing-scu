...
typedef bo::adjacency_list<bo::vecS, bo::vecS, bo::directedS, Vertex, Edge, graph_p> graph_t;//���ڱ߻��
...
std::vector<vertex_desc> cfg_calculation(
    ...
    for (std::string line; getline(cg_distance_stream, line); ) {
        ...
        bo::algorithm::split(split, line, bo::is_any_of(","));;
        std::string st1;
        ...
        splits.push_back(st1);
        splits.push_back(split[split.size()-1]);
