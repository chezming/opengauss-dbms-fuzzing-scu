...
#define ABPATH "/home/omm/dbms/"
#define RLPATH_LIB "/corpus/initlib"
#define RLPATH_IRINFO "/corpus/IR_config"
#define RLPATH_OUTPUT "/"
...
const char* file = "//home/omm/openGauss-server/dest/bin/gs_ctl";
char* const options[] = { "/home/omm/openGauss-server/dest/bin/gs_ctl", "start", "-D", "/home/omm/data", "-Z", "single_node", "-l", "/home/omm/log/opengauss.log", NULL };
...
InputInfo* g_current_input = nullptr;
...
//**********************************
  ...
  SQLSTATUS result = g_connector.execute(g_current_sql);

  if (result == SQLSTATUS::ssConnectFailed) {
       while (1) {
          sleep(0.1);

          g_connector.start_db(file, options);
          result = g_connector.execute(g_current_sql);

          if ((result != SQLSTATUS::ssConnectFailed) || stop_soon) {
              break;
          }
      }
  }
  
  if(result == SQLSTATUS::ssServerCrash){
      status = FAULT_CRASH;
  }else{
      if(result == SQLSTATUS::ssNormal){
          vaild_flag = 1;
          total_vaild++;
      }
...
static void write_stats_file(double bitmap_cvg, double stability, double eps) {
  ...
  fprintf(f, "start_time        : %llu\n"
             ...
             queue_cycle ? (queue_cycle - 1) : 0, total_execs, total_vaild, eps,
    ...
    fprintf(f, "mutate total       : %llu\n", g_mutator.totalMutate_);
    fprintf(f, "mutate failed       : %llu\n", g_mutator.failedMutate_);
             /* ignore errors */
...
static void maybe_update_plot_file(double bitmap_cvg, double eps) {
  ...
  fprintf(plot_file, 
          "%llu, %llu, %u, %u, %u, %u, %0.02f%%, %llu, %llu, %u, %0.02f, %llu, %llu, %llu\n",
          get_cur_time() / 1000, queue_cycle - 1, current_entry, queued_paths,
          pending_not_fuzzed, pending_favored, bitmap_cvg, unique_crashes,
          unique_hangs, max_depth, eps, total_execs, total_vaild, seed_count);
...
#define SP20    SP10 SP10
  ...
  sprintf(tmp, "%s (%0.02f%%)", DI(total_vaild),
      total_execs ? ((double)total_vaild) * 100 / total_execs : (double)0);

  SAYF(bV bSTOP "  total vaild : " cRST "%-21s " bSTG bV bSTOP, tmp);

  sprintf(tmp, "%s (%0.02f%%)", DI(queued_favored),
      ((double)queued_favored) * 100 / queued_paths);

  SAYF(" favored paths : " cRST "%-22s " bSTG bV "\n", tmp);
  ...
  if (get_cur_time() - last_save_time > 60 * 60 * 1000) {
      u8* fn = alloc_printf("%s/coverage_statistics", out_dir);

      FILE* fd = fopen(fn, "a+");

      if (!fd) PFATAL("Unable to create '%s'", fn);

      ck_free(fn);

      fprintf(fd, "%d\t%d\t%d (%0.02f%%)\t%d(%0.02f%%)\n", (unsigned int)((get_cur_time() - start_time) / (60 * 60 * 1000)), queued_paths, total_vaild, total_execs ? ((double)total_vaild) * 100 / total_execs : (double)0, t_bytes, t_byte_ratio);

      fclose(fd);

      last_save_time = get_cur_time();
  }
...
EXP_ST u8 common_fuzz_stuff(char** argv, const u8* out_buf, u32 len) {
  ...
  queued_discovered += (should_keep == 1 || should_keep == 2);

  g_mutator.update_status(g_current_input, should_keep, tmp_vaild_flag);
...
static u8 fuzz_one(char** argv) {
  ...
  vector<InputInfo*> mutated_tree = g_mutator.mutate_all((const char*)out_buf);
  
  stage_max = mutated_tree.size();
  stage_cur = 0;

  for(InputInfo* input: mutated_tree){
    stage_name = "sql_validate";

    u8* fn = alloc_printf("%s/crash_input", out_dir);
    s32 fd;
    FILE* f;
    ...
    fprintf(f, "%s\n", input->input_->to_string().c_str());

    fclose(f);

    bool tmp_res = g_instantiator.instantaite_sql(input->input_);
    ...
    show_stats();
    string ir_str = input->input_->to_string();
...
EXP_ST void setup_dirs_fds(void) {
  ...
  fprintf(plot_file, "# unix_time, cycles_done, cur_path, paths_total, "
                     ...
                     "unique_hangs, max_depth, execs_per_sec, total_execs, total_vaild, seed_count\n");
}
...
int main(int argc, char** argv) {
  ...
  g_mutator.init_IR_library(INIT_LIB);
  g_mutator.init_IR_config_info(GRAMMAR_INFORMATION);

  g_connector.close_db();
  g_connector.start_db(file, options);
...
stop_fuzzing:
  ...
  //g_mutator.print_information(OUTPUT_PATH);
  g_connector.close_db();
